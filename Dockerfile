...
RUN \
	if [ "${TARGETARCH}" = "amd64" ]; then HASH="${H_AMD64_KUBECTL}"; else HASH="${H_ARM64_KUBECTL}"; fi \
	&& curl -L -o kubectl "https://dl.k8s.io/release/v${V_KUBECTL}/bin/linux/${TARGETARCH}/kubectl" \
...
RUN \
	if [ "${TARGETARCH}" = "amd64" ]; then HASH="${H_AMD64_ETCD}"; else HASH="${H_ARM64_ETCD}"; fi \
	&& curl -L -o etcd.tar.gz "https://github.com/etcd-io/etcd/releases/download/v${V_ETCD}/etcd-v${V_ETCD}-linux-${TARGETARCH}.tar.gz" \
...
RUN \
	if [ "${TARGETARCH}" = "amd64" ]; then HASH="${H_AMD64_AUGER}"; else HASH="${H_ARM64_AUGER}"; fi \
	&& curl -L -o auger.tar.gz "https://github.com/etcd-io/auger/releases/download/v${V_AUGER}/auger_${V_AUGER}_linux_${TARGETARCH}.tar.gz" \
...
RUN \
	if [ "${TARGETARCH}" = "amd64" ]; then HASH="${H_AMD64_TRIVY}"; else HASH="${H_ARM64_TRIVY}"; fi \
	&& if [ "${TARGETARCH}" = "amd64" ]; then LOCAL_ARCH=64bit; else LOCAL_ARCH=ARM64; fi \
...
RUN \
	if [ "${TARGETARCH}" = "amd64" ]; then HASH="${H_AMD64_RBACLOOKUP}"; else HASH="${H_ARM64_RBACLOOKUP}"; fi \
	&& if [ ${TARGETARCH} = amd64 ]; then LOCAL_ARCH=x86_64; else LOCAL_ARCH=arm64; fi \
